...
function EventsSet(props) {
    return props.arrayToShow.map((item, index) => <Event
        key={index}
        {...item}
        onSize={props.onSize}
    />)
...
function DevicesSlider(props) {
    ...
    return <div className="section__panel-wrapper" ref={ref}>
        <div ref={sliderRef} role="tabpanel" className='section__panel' aria-hidden='false' id={`panel_${activeTab}`} aria-labelledby={`tab_${activeTab}`}>
            <ul className="section__panel-list">
                {activeTab === "all" ? Array(2**6).fill(0).map((el, i) => <MemoizedEventsSet key={i} arrayToShow={TABS[activeTab].items} onSize={memoizedOnSize} />) : 
                <MemoizedEventsSet arrayToShow={TABS[activeTab].items} onSize={onSize} />
            }
            </ul>
        </div>
        {hasRightScroll &&
            <div className="section__arrow" onClick={() => sliderRef.current.scrollTo({left: sliderRef.current.scrollLeft + 400, behavior: "smooth"})}></div>
        }
    </div>
}
